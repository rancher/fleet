...
func (w *waiter) waitForResources(created ResourceList) error {
	...
	return wait.Poll(2*time.Second, w.timeout, func() (bool, error) {
		for _, v := range created {
			...
			case *corev1.Pod:
				pod, err := w.c.CoreV1().Pods(v.Namespace).Get(context.TODO(), v.Name, metav1.GetOptions{})
				if err != nil || !w.isPodReady(pod) {
					return false, err
				}
			case *appsv1.Deployment, *appsv1beta1.Deployment, *appsv1beta2.Deployment, *extensionsv1beta1.Deployment:
				currentDeployment, err := w.c.AppsV1().Deployments(v.Namespace).Get(context.TODO(), v.Name, metav1.GetOptions{})
				if err != nil {
					return false, err
				}
			...
			case *corev1.PersistentVolumeClaim:
				claim, err := w.c.CoreV1().PersistentVolumeClaims(v.Namespace).Get(context.TODO(), v.Name, metav1.GetOptions{})
				if err != nil {
					return false, err
				}
			...
			case *corev1.Service:
				svc, err := w.c.CoreV1().Services(v.Namespace).Get(context.TODO(), v.Name, metav1.GetOptions{})
				if err != nil {
					return false, err
				}
			...
			case *extensionsv1beta1.DaemonSet, *appsv1.DaemonSet, *appsv1beta2.DaemonSet:
				ds, err := w.c.AppsV1().DaemonSets(v.Namespace).Get(context.TODO(), v.Name, metav1.GetOptions{})
				if err != nil {
					return false, err
				}
			...
			case *appsv1.StatefulSet, *appsv1beta1.StatefulSet, *appsv1beta2.StatefulSet:
				sts, err := w.c.AppsV1().StatefulSets(v.Namespace).Get(context.TODO(), v.Name, metav1.GetOptions{})
				if err != nil {
					return false, err
				}
