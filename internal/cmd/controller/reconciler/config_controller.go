...
func (r *ConfigReconciler) Reconcile(ctx context.Context, _ ctrl.Request) (ctrl.Result, error) {
	...
	cfg, adopted, err := config.ReadConfig(cm)
	if err != nil {
		return ctrl.Result{}, err
	}
	if !adopted {
		logger.V(1).Info("ignoring config ConfigMap because the version annotation is missing or does not match the controller version; keeping current config",
			"configmap", cm.Name,
