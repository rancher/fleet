...
func TestDiff_WebhookCABundle_ControllerInjected(t *testing.T) {
	ctrl := gomock.NewController(t)
	sink := mocks.NewMockLogSink(ctrl)
	sink.EXPECT().Init(gomock.Any())
	sink.EXPECT().Enabled(gomock.Any()).Return(true).AnyTimes()
	sink.EXPECT().Info(gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes()
	...
	webhooks, _, _ := unstructured.NestedSlice(live.Object, "webhooks")
	webhook := webhooks[0].(map[string]any)
	clientConfig := webhook["clientConfig"].(map[string]any)
	clientConfig["caBundle"] = "LS0tLS1CRUdJTi1DRVJUSUZJQ0FURS0tLS0t"
	_ = unstructured.SetNestedSlice(live.Object, webhooks, "webhooks")
	...
	plan := desiredset.Plan{
		...
		Update: desiredset.PatchByGVK{
			schema.GroupVersionKind{
				...
				objectset.ObjectKey{Name: "cert-manager-webhook"}: "{}",
			},
		},
	}

	bd := &v1alpha1.BundleDeployment{}
	result, err := desiredset.Diff(logger, plan, bd, ns, desired)
	if err != nil {
	...
	key := objectset.ObjectKey{Name: "cert-manager-webhook"}

	if patches, ok := result.Update[gvk]; ok {
		if patch, exists := patches[key]; exists {
			t.Errorf("Expected no diff for controller-injected caBundle, but got patch: %s", patch)
		}
	}
}

func TestDiff_WebhookCABundle_UserManaged(t *testing.T) {
	ctrl := gomock.NewController(t)
	sink := mocks.NewMockLogSink(ctrl)
	sink.EXPECT().Init(gomock.Any())
	sink.EXPECT().Enabled(gomock.Any()).Return(true).AnyTimes()
			...
			"webhooks": []any{
				map[string]any{
					...
					"clientConfig": map[string]any{
						...
						"caBundle": "QUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQT0=",
					},
				},
			},
		},
	...
	webhook := webhooks[0].(map[string]any)
	clientConfig := webhook["clientConfig"].(map[string]any)
	clientConfig["caBundle"] = "QkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQkJCQT0="
	_ = unstructured.SetNestedSlice(live.Object, webhooks, "webhooks")
	...
	plan := desiredset.Plan{
		...
		Update: desiredset.PatchByGVK{
			schema.GroupVersionKind{
				...
				objectset.ObjectKey{Name: "my-webhook"}: "{}",
			},
		},
	}
	...
	key := objectset.ObjectKey{Name: "my-webhook"}

	if patches, ok := result.Update[gvk]; ok {
		if _, exists := patches[key]; !exists {
			t.Error("Expected diff for user-managed caBundle drift, but no patch found")
		}
	} else {
		t.Error("Expected diff for user-managed caBundle drift, but Update is empty")
	}
}

func TestDiff_SimpleWebhookChange(t *testing.T) {
	ctrl := gomock.NewController(t)
	sink := mocks.NewMockLogSink(ctrl)
	sink.EXPECT().Init(gomock.Any())
	sink.EXPECT().Enabled(gomock.Any()).Return(true).AnyTimes()
	...
	if patches, ok := result.Update[gvk]; ok {
		if patch, exists := patches[key]; exists {
			t.Logf("Detected diff (good!): %s", patch)
		} else {
			t.Error("Expected diff for webhook rules change, but no patch found")
		}
	} else {
		t.Error("Expected diff for webhook rules change, but Update is empty")
	}
}

func TestDiff_WebhookCABundle_EmptyWebhookFiltered(t *testing.T) {
	ctrl := gomock.NewController(t)
	sink := mocks.NewMockLogSink(ctrl)
	sink.EXPECT().Init(gomock.Any())
	sink.EXPECT().Enabled(gomock.Any()).Return(true).AnyTimes()
		...
		Object: map[string]any{
			...
			"webhooks": []any{
				map[string]any{
					"name": "test.example.com",
					"clientConfig": map[string]any{
						"caBundle": "LS0tLS1CRUdJTi1DRVJUSUZJQ0FURS0tLS0t",
						// Only caBundle, nothing else in clientConfig
					},
				},
			},
		},
	...
	plan := desiredset.Plan{
		...
		Update: desiredset.PatchByGVK{
			schema.GroupVersionKind{
				...
				objectset.ObjectKey{Name: "test-webhook"}: "{}",
			},
		},
	}
	...
	key := objectset.ObjectKey{Name: "test-webhook"}

	if patches, ok := result.Update[gvk]; ok {
		if patch, exists := patches[key]; exists {
			t.Errorf("Expected empty patch after filtering empty webhooks, but got: %s", patch)
		}
	}
}

func TestDiff_WebhookCABundle_WholesaleReplacement(t *testing.T) {
	ctrl := gomock.NewController(t)
	sink := mocks.NewMockLogSink(ctrl)
	sink.EXPECT().Init(gomock.Any())
	sink.EXPECT().Enabled(gomock.Any()).Return(true).AnyTimes()
			...
			"webhooks": []any{
				map[string]any{
					...
					"clientConfig": map[string]any{
						...
						"caBundle": "Y2VydC1tYW5hZ2VyLWluamVjdGVk", // controller-injected
					},
					"sideEffects": "None",
				},
			},
	...
	if patches, ok := result.Update[gvk]; ok {
		if patch, exists := patches[key]; exists {
			t.Errorf("Expected no diff for controller-injected caBundle (wholesale replacement), but got patch: %s", patch)
		}
	}
}
