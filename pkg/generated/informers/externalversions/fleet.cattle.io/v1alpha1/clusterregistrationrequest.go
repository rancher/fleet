...
func NewClusterRegistrationRequestInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredClusterRegistrationRequestInformer(client, namespace, resyncPeriod, indexers, nil)
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.FleetV1alpha1().ClusterRegistrationRequests(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.FleetV1alpha1().ClusterRegistrationRequests(namespace).Watch(context.TODO(), options)
			},
		},
		&fleetcattleiov1alpha1.ClusterRegistrationRequest{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *clusterRegistrationRequestInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredClusterRegistrationRequestInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *clusterRegistrationRequestInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&fleetcattleiov1alpha1.ClusterRegistrationRequest{}, f.defaultInformer)
}
