...
func CollectClusterGroupMetrics(clusterGroup *fleet.ClusterGroup) {
	if !enabled {
		return
	}
	...
	clusterGroupClusterCount.With(labels).Set(float64(clusterGroup.Status.ClusterCount))
	clusterGroupNonReadyClusterCount.With(labels).Set(float64(clusterGroup.Status.NonReadyClusterCount))
	clusterGroupResourcesDesiredReady.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.DesiredReady))
	clusterGroupResourcesMissing.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.Missing))
	clusterGroupResourcesModified.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.Modified))
	clusterGroupResourcesNotReady.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.NotReady))
	clusterGroupResourcesOrphaned.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.Orphaned))
	clusterGroupResourcesReady.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.Ready))
	clusterGroupResourcesUnknown.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.Unknown))
	clusterGroupResourcesWaitApplied.With(labels).Set(float64(clusterGroup.Status.ResourceCounts.WaitApplied))
	clusterGroupDesiredReadyBundles.With(labels).Set(float64(clusterGroup.Status.Summary.DesiredReady))
	clusterGroupReadyBundles.With(labels).Set(float64(clusterGroup.Status.Summary.Ready))
