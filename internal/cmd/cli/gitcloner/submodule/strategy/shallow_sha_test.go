...
func TestShallowSHAStrategy_Type(t *testing.T) {
	s := NewShallowSHAStrategy(nil)
	if s.Type() != capability.StrategyShallowSHA {
		t.Errorf("expected %v, got %v", capability.StrategyShallowSHA, s.Type())
	}
}

func TestShallowSHAStrategy_Success(t *testing.T) {
	fetchCalled := false
	checkoutCalled := false
	expectedHash := plumbing.NewHash("abc123")

	s := &ShallowSHAStrategy{
...
func TestShallowSHAStrategy_FetchError(t *testing.T) {
	s := &ShallowSHAStrategy{
		fetchFunc: func(ctx context.Context, r *git.Repository) error {
...
func TestShallowSHAStrategy_CheckoutError(t *testing.T) {
	s := &ShallowSHAStrategy{
		fetchFunc: func(ctx context.Context, r *git.Repository) error {
			return nil
		},
...
func TestShallowSHAStrategy_ContextCancellation(t *testing.T) {
...
func TestNewShallowSHAStrategy(t *testing.T) {
	// auth is nil because we're not testing actual connections
	...
	// fetchFunc is nil by default, uses defaultFetch
}
