...
// ClusterRegistrationHandler is function for performing any potential modifications to a ClusterRegistration resource.
type ClusterRegistrationHandler func(string, *v1alpha1.ClusterRegistration) (*v1alpha1.ClusterRegistration, error)

// ClusterRegistrationIndexer computes a set of indexed values for the provided object.
type ClusterRegistrationIndexer func(obj *v1alpha1.ClusterRegistration) ([]string, error)

// ClusterRegistrationGenericController wraps wrangler/pkg/generic.Controller so that the function definitions adhere to ClusterRegistrationController interface.
type ClusterRegistrationGenericController struct {
	generic.ControllerInterface[*v1alpha1.ClusterRegistration, *v1alpha1.ClusterRegistrationList]
}

// OnChange runs the given resource handler when the controller detects a resource was changed.
func (c *ClusterRegistrationGenericController) OnChange(ctx context.Context, name string, sync ClusterRegistrationHandler) {
	c.ControllerInterface.OnChange(ctx, name, generic.ObjectHandler[*v1alpha1.ClusterRegistration](sync))
}

// OnRemove runs the given object handler when the controller detects a resource was changed.
func (c *ClusterRegistrationGenericController) OnRemove(ctx context.Context, name string, sync ClusterRegistrationHandler) {
	c.ControllerInterface.OnRemove(ctx, name, generic.ObjectHandler[*v1alpha1.ClusterRegistration](sync))
}

// Cache returns a cache of resources in memory.
func (c *ClusterRegistrationGenericController) Cache() ClusterRegistrationCache {
	return &ClusterRegistrationGenericCache{
		c.ControllerInterface.Cache(),
	}
}

// ClusterRegistrationGenericCache wraps wrangler/pkg/generic.Cache so the function definitions adhere to ClusterRegistrationCache interface.
type ClusterRegistrationGenericCache struct {
	generic.CacheInterface[*v1alpha1.ClusterRegistration]
}

// AddIndexer adds  a new Indexer to the cache with the provided name.
// If you call this after you already have data in the store, the results are undefined.
func (c ClusterRegistrationGenericCache) AddIndexer(indexName string, indexer ClusterRegistrationIndexer) {
	c.CacheInterface.AddIndexer(indexName, generic.Indexer[*v1alpha1.ClusterRegistration](indexer))
}
...
func FromClusterRegistrationHandlerToHandler(sync ClusterRegistrationHandler) generic.Handler {
	return generic.FromObjectHandlerToHandler(generic.ObjectHandler[*v1alpha1.ClusterRegistration](sync))
}
