...
var _ = Describe("git fetch's LatestCommit tests", func() {
	...
	It("uses the injected TokenProvider when one is supplied (GitHub-App workflow)", func() {
		gr := &fleetv1.GitRepo{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-gitrepo",
				Namespace: "test-ns",
			...
			Spec: fleetv1.GitRepoSpec{
				Repo:             fakeGithub.URL,
				Branch:           "master",
				ClientSecretName: "gh-app-secret",
			},
		}
		...
		ghSecret := &corev1.Secret{
			...
			Data: map[string][]byte{
				...
				"github_app_private_key":     []byte("dummy-pem"),
			},
		}

		c := newTestClient(ghSecret)

		const tokenVal = "injected-provider-token"
		f := git.Fetch{
	...
	It("propagates an error when the injected TokenProvider fails", func() {
		gr := &fleetv1.GitRepo{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-gitrepo",
				Namespace: "test-ns",
		...
		providerErr := errors.New("token provider boom")
		f := git.Fetch{
			TokenProvider: func(ctx context.Context) (string, error) {
				return "", providerErr
			},
		...
		commit, err := f.LatestCommit(context.Background(), gr, newTestClient())
		Expect(err).To(HaveOccurred())
