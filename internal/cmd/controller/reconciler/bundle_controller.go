...
func (r *BundleReconciler) handleDelete(ctx context.Context, logger logr.Logger, req ctrl.Request, bundle *fleet.Bundle) (ctrl.Result, error) {
	...
	return ctrl.Result{}, r.deleteOCIArtifact(ctx, bundle)
}
...
func (r *BundleReconciler) createBundleDeployment(
	...
	op, err := controllerutil.CreateOrUpdate(ctx, r.Client, bd, func() error {
		...
		// check if there's any previous OCI secret that can be purged
...
// deleteOCIArtifact deletes the Bundle OCI artifact if it was used
func (r *BundleReconciler) deleteOCIArtifact(ctx context.Context, bundle *fleet.Bundle) error {
	if bundle.Spec.ContentsID == "" {
		return nil
	}
