...
func (u *Upgrade) performUpgrade(originalRelease, upgradedRelease *release.Release) (*release.Release, error) {
	...
	err = target.Visit(setMetadataVisitor(upgradedRelease.Name, upgradedRelease.Namespace, true, u.Adopt))
	if err != nil {
...
func recreate(cfg *Configuration, resources kube.ResourceList) error {
	for _, res := range resources {
		...
		pods, err := client.CoreV1().Pods(res.Namespace).List(context.TODO(), metav1.ListOptions{
