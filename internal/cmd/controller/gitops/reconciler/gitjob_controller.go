...
func (r *GitJobReconciler) newJobSpec(ctx context.Context, gitrepo *v1alpha1.GitRepo) (*batchv1.JobSpec, error) {
	...
	saName := names.SafeConcatName("git", gitrepo.Name)
	logger := log.FromContext(ctx)
	args, envs := argsAndEnvs(gitrepo, logger.V(1).Enabled(), r.KnownHosts)
...
func argsAndEnvs(gitrepo *v1alpha1.GitRepo, debug bool, knownHosts KnownHostsGetter) ([]string, []corev1.EnvVar) {
	args := []string{
		"fleet",
		"apply",
	}
