...
func CollectGitRepoMetrics(gitrepo *fleet.GitRepo) {
	if !enabled {
		return
	}
	...
	gitrepoDesiredReadyClusters.With(labels).Set(float64(gitrepo.Status.DesiredReadyClusters))
	gitrepoReadyClusters.With(labels).Set(float64(gitrepo.Status.ReadyClusters))

	gitrepoResourcesMissing.With(labels).Set(float64(gitrepo.Status.ResourceCounts.Missing))
	gitrepoResourcesModified.With(labels).Set(float64(gitrepo.Status.ResourceCounts.Modified))
	gitrepoResourcesNotReady.With(labels).Set(float64(gitrepo.Status.ResourceCounts.NotReady))
	gitrepoResourcesOrphaned.With(labels).Set(float64(gitrepo.Status.ResourceCounts.Orphaned))
	gitrepoResourcesDesiredReady.With(labels).Set(float64(gitrepo.Status.ResourceCounts.DesiredReady))
	gitrepoResourcesReady.With(labels).Set(float64(gitrepo.Status.ResourceCounts.Ready))
	gitrepoResourcesUnknown.With(labels).Set(float64(gitrepo.Status.ResourceCounts.Unknown))
	gitrepoResourcesWaitApplied.With(labels).Set(float64(gitrepo.Status.ResourceCounts.WaitApplied))
