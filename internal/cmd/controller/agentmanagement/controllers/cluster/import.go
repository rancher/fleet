...
func (i *importHandler) importCluster(cluster *fleet.Cluster, status fleet.ClusterStatus) (fleet.ClusterStatus, error) {
	...
	if schedulingCustomization := cluster.Spec.AgentSchedulingCustomization; schedulingCustomization != nil {
		if schedulingCustomization.PriorityClass != nil {
			priorityClassName = scheduling.FleetAgentPriorityClassName
			objs = append(objs, scheduling.PriorityClass(schedulingCustomization.PriorityClass))
		}

		if schedulingCustomization.PodDisruptionBudget != nil {
			pdb, err := scheduling.PodDisruptionBudget(agentNamespace, schedulingCustomization.PodDisruptionBudget)
			if err != nil {
				return status, err
			}
