...
func reloadConfig(namespace string, configMap *v1.ConfigMap) (*v1.ConfigMap, error) {
	...
	cfg, adopted, err := config.ReadConfig(configMap)
	if err != nil {
		return configMap, err
	}
	if !adopted {
		// ConfigMap is annotated for a different Fleet version (or missing a version annotation);
		// keep the running config and do not re-trigger config-change callbacks.
		return configMap, nil
	}
