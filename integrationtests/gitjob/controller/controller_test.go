	...
	When("a new GitRepo is created", func() {
		...
		When("a job is created without a specified CA bundle, but Rancher has secrets containing a CA bundle", func() {
			...
			It("creates secrets for the CA bundle", func() {
				...
				found = false
				for _, v := range volumes {
					if v.Name == "rancher-helm-secret-cert" {
						found = true
						break
					}
				}
				...
				Expect(strings.Join(containers[0].Args, " ")).To(ContainSubstring("--cacerts-file /etc/rancher/certs/cacerts"))

				found = false
				for _, vm := range volumeMounts {
					if vm.Name == "rancher-helm-secret-cert" {
						found = true
