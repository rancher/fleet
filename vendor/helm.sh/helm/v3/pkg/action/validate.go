...
func existingResourceConflict(resources kube.ResourceList, releaseName, releaseNamespace string, adopt bool) (kube.ResourceList, error) {
	...
	err := resources.Visit(func(info *resource.Info, err error) error {
		...
		if err := checkOwnership(existing, releaseName, releaseNamespace, adopt); err != nil {
...
func checkOwnership(obj runtime.Object, releaseName, releaseNamespace string, adopt bool) error {
	...
	var errs []error
	if err := requireValue(lbls, appManagedByLabel, appManagedByHelm, adopt); err != nil {
		errs = append(errs, fmt.Errorf("label validation error: %s", err))
	}
	if err := requireValue(annos, helmReleaseNameAnnotation, releaseName, adopt); err != nil {
		errs = append(errs, fmt.Errorf("annotation validation error: %s", err))
	}
	if err := requireValue(annos, helmReleaseNamespaceAnnotation, releaseNamespace, adopt); err != nil {
		errs = append(errs, fmt.Errorf("annotation validation error: %s", err))
	}
...
func requireValue(meta map[string]string, k, v string, adopt bool) error {
	actual, ok := meta[k]
	if !ok {
		if adopt {
			return nil
...
func setMetadataVisitor(releaseName, releaseNamespace string, force, adopt bool) resource.VisitorFunc {
	return func(info *resource.Info, err error) error {
		...
		if !force {
			if err := checkOwnership(info.Object, releaseName, releaseNamespace, adopt); err != nil {
