...
func getZotConfig() (string, error) {
	...
	reader, _, err := getEnvVarUserAndPassword("CI_OCI_READER_USERNAME", "CI_OCI_READER_PASSWORD")
	if err != nil {
		return "", err
	}
	writer, _, err := getEnvVarUserAndPassword("CI_OCI_USERNAME", "CI_OCI_PASSWORD")
	if err != nil {
		return "", err
	}
...
func getEnvVarUserAndPassword(username, password string) (string, string, error) {
	...
	if !ok {
		return "", "", fmt.Errorf("%s is not set", password)
	}
